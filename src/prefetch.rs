...
impl<'a> PrefetchBatcher<'a> {
    ...
    pub(crate) fn push(&mut self, range: Range<usize>) {
        ...
        let Some(range) = clamp_range(range, self.mmap.len()) else { return; };
    ...
    pub(crate) fn push(&mut self, _range: Range<usize>) { /* no-op */}
    ...
    #[cfg(not(windows))]
    pub(crate) fn flush(&mut self) { /* no-op */ }
}
